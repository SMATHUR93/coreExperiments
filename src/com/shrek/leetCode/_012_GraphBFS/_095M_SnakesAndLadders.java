...
public class _095M_SnakesAndLadders {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("snakesAndLadders for board = "+Arrays.toString(board)+" , equals "+ snakesAndLadders(board));
        // 4
        System.out.println();
    ...
    public static int snakesAndLadders(int[][] board) {
        ...
        for(int row=n-1;row>=0;row--){
            for(int column: columns){
                cells.put(label++, new Integer[]{row, column});
            }
        ...
        while(!queue.isEmpty()){
            ...
            int neighbours = Math.min(currentNode + 6, n*n);
            System.out.println(" Debugging, currentNode = "+currentNode);
            for(int nextNode=currentNode+1;nextNode<=neighbours;nextNode++){
                int row = cells.get(nextNode)[0];
                int col = cells.get(nextNode)[1];
                int destination;
                if(board[row][col]==-1){
                ...
                // System.out.println(" Debugging, nextNode = "+nextNode+" , destination = "+destination);
